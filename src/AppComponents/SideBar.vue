<template>
    <div class="flex">
        <!-- Mobile Overlay -->
        <div v-if="sidebarOpen" @click="sidebarOpen = false" class="fixed inset-0  bg-opacity-50 lg:hidden">
        ...
        <div :class="[
            ...
            <nav class="mt-8 p-2">
                <router-link v-for="item in sidebarItems" :key="item.name" :to="item.link" class=""
...
const secondLastPathName = pathSegment[pathSegment.length - 2]
...
const sidebarItems = ref([
    ...
    { name: 'Monitoring and Alert', icon: ServiceIcon, activeIcon: ServiceIConActive, active: false, link: '/monitoring-altering' },
    { name: 'Analyst Desk', icon: ServiceIcon, activeIcon: ServiceIConActive, active: false, link: '/analyst-desk-management' },
    { name: 'Notifications', icon: NotificationIcon, active: false,link: ''}
])
...
onMounted(() => {
    ...
    sidebarItems.value.forEach(i => {
        i.link.slice(1) == lastPathName ? i.active = true : i.active = false
    });
    if(secondLastPathName == 'ticket-overview'){
        sidebarItems.value[0].active = true
    }
})
</script>